...
export default function TableAsset(
    { data, totalPrice, trackStatus, ModalView, ModalEdit, fetchData }:
        { data: any, totalPrice: number, trackStatus: any, ModalView: any, ModalEdit: any, fetchData: any }
) {
    return (
        <>
            <Table aria-label="table asset">
                ...
                <TableBody items={data.Equipment} emptyContent={"No rows to display."}>
                    {data?.Table_ITC_0001?.map((item: any, index: number) => (
            ...
            {data.Track_Doc
                ...
                .map((index: { step: number, remark: string }) => (
