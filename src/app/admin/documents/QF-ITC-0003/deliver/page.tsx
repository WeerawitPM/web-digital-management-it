...
export default function Home() {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get('/api/admin/documents/QF-ITC-0003/deliver'); // เรียกใช้งาน API ที่เส้นทาง '/api'
                    ...
                    >
                        {data?.map((item: any, index: number) => (
                            <TableRow key={item.key}>
                                ...
                                <TableCell>
                                    <Link href={`/admin/documents/QF-ITC-0003/deliver/doc_no/${item.ref_no}`} className="text-blue-500">{item.ref_no}</Link>
                                </TableCell>
                                <TableCell>
