...
export default function Home() {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get('/api/admin/documents/QF-ITC-0003/summary'); // เรียกใช้งาน API ที่เส้นทาง '/api'
    ...
    return (
        <>
            <HeaderMain title="| สรุปผลการดำเนินงาน" />
                    ...
                    >
                        {data?.map((item: any, index: number) => (
                            <TableRow key={item.key}>
                                ...
                                <TableCell>
                                    <Link href={`/admin/documents/QF-ITC-0003/summary/doc_no/${item.ref_no}`} className="text-blue-500">{item.ref_no}</Link>
                                </TableCell>
                                <TableCell>
                                    {item.start_date && new Date(item.start_date).toLocaleDateString('th-TH')}
                                </TableCell>
                                <TableCell>
                                    {item.Table_ITC_0003[0]?.request_by.username}
                                </TableCell>
                                <TableCell>
