...
export default function Component(
    { data, step, status, isLoading }:
        { data: any, step: any, status: number, isLoading: boolean }
) {
                    ...
                    >
                        {data?.filter((item: any) => item.status === status).map((item: any, index: number) => (
                            <TableRow key={item.key}>
                                ...
                                <TableCell>
                                    <Link
                                        href={`/admin/documents/QF-ITC-0006/doc_no/${item.ref_no}`}
                                ...
                                <TableCell>
                                    {item.Table_ITC_0006[0].request_by.username}
                                </TableCell>
                                <TableCell>
