...
export default function Home({ params }: { params: any }) {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get(`/api/documents/QF-ITC-0006/doc_no?doc_no=${doc_no}`);
    ...
    return (
        <Component data={data} steps={steps} statusStep={statusStep} doc_no={doc_no} trackStatus={trackStatus} fetchData={fetchData} />
    );
}